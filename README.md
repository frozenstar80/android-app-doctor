...
One line in your `Application` gives you a draggable floating button on every screen that opens a live dashboard of **device info, app info, memory, FPS and CPU** — and it compiles to **nothing** in release builds.
...
> Phase 1 (MVP). Built with Kotlin, Jetpack Compose and Clean Architecture, and designed from day one to grow into a full in‑app debugging suite (Network Inspector, Room Inspector, Compose Inspector, plugin system).
...
AppDoctor.install(
    ...
    config = AppDoctorConfig(
        ...
        // overlayFactory = MyOverlayFactory(),  // inject a custom UI (DI / tests)
        // plugins = listOf(MyPlugin()),         // future inspectors
    ),
)
```
...
- 🌐 **Network Inspector** — capture and browse HTTP traffic.
